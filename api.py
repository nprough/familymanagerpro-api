...
@app.route('/tasklist/<item_id>', methods=['GET'])
def get_item(item_id):
    items = mongo.db.tasklist
    output = ""
    for i in items.find():
...
def create_item():
    if not request.json or not 'name' in request.json:
        abort(400)
    newitem = {'name': request.json['name']}
    mongo.db.tasklist.insert_one(newitem)
    return jsonify({'new_item': {'name': request.json['name']}}), 201
...
@app.route('/tasklist/<item_id>', methods=['DELETE'])
def delete_item(item_id):
    mongo.db.tasklist.delete_one({'_id': ObjectId(item_id)})
    return jsonify({'deleted': True})
